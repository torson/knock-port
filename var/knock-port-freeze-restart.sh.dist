...
# Sending request and checking if knockport log file got updated - if timestamp is older than TTL then it has not updated.
...
if curl -s -m 1 -i -d 'app=healthcheck&access_key=1234567890' http://${KNOCKPORT_HOST}:${KNOCKPORT_HTTP_PORT}${KNOCKPORT_HTTP_URL_PATH} | grep "200 OK" >/dev/null ; then
    ...
    log "/usr/sbin/service knockport restart"
    /usr/sbin/service knockport restart
fi
